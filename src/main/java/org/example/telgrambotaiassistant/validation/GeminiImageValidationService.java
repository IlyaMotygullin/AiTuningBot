...
public class GeminiImageValidationService implements ImageValidationService {
    ...
    public GeminiImageValidationService(
    ...
    public ValidationResult validate(byte[] imageBytes, String mimeType, ImageType type) {
        ValidationResult basic = BasicImageValidator.validate(imageBytes, mimeType);
        if (!basic.valid()) {
            return basic;
        }
        if (geminiClient.resolveApiKey() == null) {
            return ValidationResult.fail("""
                    Не настроен API-ключ ProxyAPI.
                    Добавьте в application.yaml: bot.ai.api-key: ВАШ_КЛЮЧ
                    """);
        }
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                boolean isQuota = geminiClient.isQuotaError(ex);
    ...
    private ValidationResult validateWithGemini(byte[] imageBytes, String mimeType, ImageType type) {
        ...
        body.set("contents", geminiClient.userContent(
                geminiClient.inlineImagePart(imageBytes, mimeType),
                geminiClient.textPart(promptFor(type))
        ));
    ...
    private static String promptFor(ImageType type) {
    ...
    private ValidationResult handleApiError(RestClientException ex, boolean isQuota) {
        ...
        if (isQuota) {
            return ValidationResult.fail("""
                    Сейчас исчерпана квота ProxyAPI/Google для проверки фото.
                    Попробуйте позже или пополните баланс на proxyapi.ru.
                    """);
        }
